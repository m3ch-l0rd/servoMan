...
pwm_2 = PWM(Pin(16)) #right arm
pwm_2.freq(50)
pwm_2.duty_ns(MID)

pwm_3 = PWM(Pin(17)) #head
pwm_3.freq(50)
pwm_3.duty_ns(MID)
...
def core1_thread():
    ...
    while True:
        ...
        utime.sleep(0.5)
        print('.*.*.*')
        if lightPowerValue < 2.4:
            print('low light operating conditions detected')
            utime.sleep(0.5)
            print('.*.*.*')
            print('activating lightsOnly mode')
            utime.sleep(0.5)
            print('.*.*.*')
            for i in range(10):
...
#random by function from functions array
#while True:
